...
/**
 * JUnit Test of OrderDateDescComparator.
 *
 * @author Mischa Gruber
 */
public class OrderDateDescComparatorTest {

    @Test
    public void testSorting() {
        List<OrderDTO> orderDTOs = new ArrayList<>();
        OrderDTO order1 = new OrderDTO(1);
        order1.setDate(new Timestamp(2018,6,10,17,45,0,0));
        OrderDTO order2 = new OrderDTO(2);
        order2.setDate(new Timestamp(2018,6,12,17,45,0,0));
        orderDTOs.add(order1);
        orderDTOs.add(order2);

        Comparator<OrderDTO> comparator = new OrderDateDescComparator();
